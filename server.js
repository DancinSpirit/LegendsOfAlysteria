...
app.post("/component/:component", async function(req,res){
    ...
    if(req.body.model){
        if(req.params.component.toLowerCase()=="basic-sheet"){
            ...
            model = await eval(`db.${req.body.model.name}.findById('${req.body.model.id}').populate({path:'combatStyles',populate: {path:'character weapons armor fightingStyles weaponTypes weaponStyle',populate:{path: 'traits.combatAbilities advantageOver weakAgainst info knowledgeTree passiveAbilities specialAbility', populate:{path:'generalKnowledge specializedKnowledge highlySpecializedKnowledge bonusKnowledge skills specialties', populate:{path: 'info'}}}}})`);
